...
После завершения проверки группы (или серии проверок) — зафиксировать изменения:

```
git add triz-solver-spec.html
git commit -m "review(#{sysId}): результаты аудита — X verified, Y fix_required"
git push
```

**Формат коммита:** `review(#48): аудит Фазы 0 — 13 verified`
...
- **После завершения проверки группы** — коммитить и пушить изменения HTML
